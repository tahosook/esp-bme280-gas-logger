...
4. **エラーログ履歴の確認 (`debugTest_showErrorLogs`)**:
   - スクリプトプロパティ（`ERROR_LOG_ENTRIES`）に保存されている直近のエラーログ（最大100件）を整形してログ出力。
5. **エラーログ履歴の消去 (`debugTest_clearErrorLogs`)**:
   - スクリプトプロパティに蓄積されたエラーログ履歴を全件消去。
6. **センサーデータ書き込みシミュレーション (`debugTest_simulateSensorPost`)**:
