...
GAS側で現在時刻を`Asia/Tokyo`として生成し、`yyyy-MM-dd HH:mm:ss`形式（例：`2026-08-10 01:42:09`）で保存する。`T`やタイムゾーン表記（`JST`、`+09:00`など）は付けない。次の列順で1行追記する。
...
- 最終行の測定値（temp / press / hum）が今回のリクエストと一致する
- 最終行の記録時刻が現在時刻から規定秒数（時間窓。Phase 7 の決定Issueで確定、目安180秒程度）以内である
...
現状の保存列は `日時 | temp | press | hum` の4列で実装・検証済みであり、これを維持する。
`flag`（空 / `dup` / `anomaly`）・`device_id` 列の追加や、シート名を `Sheet1` から `DATA` へ変更するかは、
Phase 7 の決定Issueで確定する（未実装）。
