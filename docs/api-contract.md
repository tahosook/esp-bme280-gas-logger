...
保存列は `日時 | temp | press | hum` の4列を維持する。シート名は `Sheet1` → `DATA` に変更し、`日時` は Date値＋表示形式にする（Phase 7で決定）。
`flag` 列（anomaly記録用）は監視の異常値検知（Phase 10/11）実装時に追加、`device_id` 列は複数台化時に追加する（ともに未実装）。
