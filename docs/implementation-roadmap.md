...
- 列追加: Phase 7 では見送り。`device_id` は複数台化時、`flag` は異常値検知（Phase 10/11）実装時に追加
- 重複判定の時間窓: **180秒**
...
- 動作: 未受信なら1回だけLINE通知 → 復帰でリセット → 再び未受信で再通知
...
| 依存関係 | なし |
| 前提・未決事項 | 既存4列（日時/temp/press/hum）は維持。日時は **Date値＋表示形式**、シート名は **`DATA`**（決定済み） |
...
| --- | --- |
| 種別 | 実装 |
| 目的 | Configシート（採用済み）、ErrorLog、日次集計（Daily 1日1行）を実装する |
...
| --- | --- |
| 種別 | 実装 |
| 目的 | DATAへの追記がしきい値（既定 **3日**、Configキー `WATCHDOG_TIMEOUT_MIN=4320`、Configシートで調整可）を超えて途絶えたら、**1回だけ** LINE通知する |
| 対象ファイル | `gas/Monitor.gs` |
| 依存関係 | Phase 11 |
| 前提・未決事項 | 時間主導トリガー（1日1回など）でDATA最終日時を定期評価し、通知状態は Script Properties で管理 |
...
| 依存関係 | Phase 15 |
| 前提・未決事項 | 前月分のDaily行を集計し、Monthlyへ1行追記。列は `年月 | temp_avg/min/max | hum_avg/min/max | press_avg/min/max | days_count` |
