...
本プロジェクトは、基盤移行（Phase 0〜6、`v1.0.0-stable`）および日次集計・環境監視・LINE Bot の拡張（Phase 7〜19、`v1.1.0-stable` + PR #24）を完了し、現在は内部品質改善・リファクタリング（Phase 20〜26）を進める。

## 現行ドキュメント（Active）

| 文書 | 内容 | 対象 |
| --- | --- | --- |
| [implementation-roadmap.md](implementation-roadmap.md) | 実装ロードマップ（正本）。Phase 0-19完了＋Phase 20-26リファクタリング | 全体 |
...
| [implementation-tasks.md](implementation-tasks.md) | タスク一覧（Phase 7-19完了、Phase 20-26リファクタリング） | 実装 |
